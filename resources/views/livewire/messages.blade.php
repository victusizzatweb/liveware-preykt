                    ...
                    <ul class="list-group list-group-flush">
                        ...
                        <a wire:click="getUser({{$user->id}})" class="text-dark link">
                            <li class="list-group-item">
                                <img src="image/icon.png" class="img-fluid avatar" alt="ewewewe">
                                   
                                        @if ($user->is_online==true)
                                        <i class="fa fa-circle text-success online-icon p-1"></i>
                                        @else
                                        <i class="fa fa-circle text-warning online-icon p-1"></i>
                                        @endif
                                        {{$user->name}}
                                        @if (filled($not_seen))
                                            <div class="badge badge-success rounded m-1">
                                                {{$not_seen->count()}}
                                            </div>
                                        @endif
            ...
            <div class="card">
                ...
                <div class="card-footer">
                    <form wire:submit.prevent="SendMessage" >
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                 <input  type="text" wire:model='message' class="form-control input shadow-none w-100 d-inline-block" placeholder="Typing" required>
                            </div>
                            <div class="col-md-4">
                                <button type="submit" class="btn btn-primary w-100 d-inline-block"><i class="far fa-paper-plane"></i> Send</button>
                            </div>
                        </div>
                    </form>
                </div>
